...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime;  // Last block time that BONEs distribution occurs.
    ...
    // set a max bone per second, which can never be higher than 1 per second
    ...
    // Changes bone token reward per second, with a cap of maxbone per second
    // Good practice to update pools without messing up the contract
    function setBooPerSecond(uint256 _bonePerSecond) external onlyOwner {
        require(_bonePerSecond <= maxBooPerSecond, "setBooPerSecond: too many bones!");

        // This MUST be done or pool rewards will be calculated with new bone per second
    ...
    function pendingBONE(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 boneReward = multiplier.mul(bonePerSecond).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        bone.mint(devaddr, boneReward.div(10));
    ...
    // Deposit LP tokens to MasterChef for BONE allocation.
